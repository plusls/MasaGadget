...
public class RenderUtil {
    ...
    public static void renderTextOnEntity(MatrixStack matrixStack, Entity entity,
        ...
        if (entityRenderDispatcher.getSquaredDistanceToCamera(entity) <= 4096.0D) {
            ...
            float h = (float) (-client.textRenderer.getStringWidth(text.asFormattedString()) / 2);
            client.textRenderer.draw(text.asFormattedString(), h, 0, 553648127, false, lv, vertexConsumerProvider, false, k, 0xf00000);
            client.textRenderer.draw(text.asFormattedString(), h, 0, -1, false, lv, vertexConsumerProvider, false, 0, 0xf00000);
            matrixStack.pop();
        }
    }
}
